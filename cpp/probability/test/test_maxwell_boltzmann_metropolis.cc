...
int main(int argc, char** argv) {
  ...
  for( int i=0; i<50000; i++ ) 
    x_metro, sample_pdf_metropolis( &pdf_maxwell, (double*)&params, range_i, range_f, x_metro);

  for( int i=0; i<200000; i++ ) { 
    x_reject = sample_pdf_rejection( &pdf_maxwell, (double*)&params, range_i, range_f);
    x_metro  = sample_pdf_metropolis( &pdf_maxwell, (double*)&params, range_i, range_f, x_metro);
    printf( "%d %lf %lf\n", i, x_reject, x_metro);
  }
