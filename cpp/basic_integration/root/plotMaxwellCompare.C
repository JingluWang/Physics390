...
void
plotMaxwellCompare(int nsamples) {
  ...
  TH1F * h_naive = new TH1F(mystr,mystr,100,0.,2.);
  sprintf(mystr, "t%d_naive", nsamples);    
  TTree * t_naive = new TTree(mystr,mystr);
  sprintf(mystr, "./data/MB-integral_1D_Naive_ntrials_%d_samples_%d.dat", trials, nsamples);    
  t_naive->ReadFile(mystr, "l0/C:trial/F:l1/C:integral/F:l2/C:err/F:l3/C:Nevents/I");
  sprintf(mystr, "integral>>h%d_naive", nsamples);
  ...
  TH1F * h_meanvar = new TH1F(mystr,mystr,100,0.,2.);  
  sprintf(mystr, "t%d_meanvar", nsamples);    
  TTree * t_meanvar = new TTree(mystr,mystr);
  sprintf(mystr, "./data/MB-integral_1D_MeanVariance_ntrials_%d_samples_%d.dat", trials, nsamples);    
  t_meanvar->ReadFile(mystr, "l0/C:trial/F:l1/C:integral/F:l2/C:err/F:l3/C:Nevents/I");
  sprintf(mystr, "integral>>h%d_meanvar", nsamples);
  t_meanvar->Draw(mystr,"","goff");
  h_meanvar->Print();


  TCanvas * c = new TCanvas("c","c", 0, 0, 700, 600);
  TH1F* frame = (TH1F*)(c->DrawFrame(0.,0.1,2.,1.1*TMath::Max(h_naive->GetMaximum(),h_meanvar->GetMaximum())));
